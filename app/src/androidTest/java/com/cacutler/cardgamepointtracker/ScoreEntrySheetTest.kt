...
class ScoreEntrySheetTest {
    ...
    fun setup() = runBlocking {
        ...
        // Create the game (returns Unit, not the ID)
        repository.createGame("Test Game", listOf("Alice"))

        // Get the game that was just created
        val game = repository.getActiveGames().first().firstOrNull()
            ?: throw IllegalStateException("Game was not created")

        // Now get the players using the actual game ID
        val players = repository.getPlayersForGame(game.game.id).first()
    ...
    fun scoreEntrySheet_switchesToSubtract() {
        ...
        // Click subtract
        composeTestRule.onNodeWithText("Subtract Points").performClick()

        // Verify quick add buttons show negative
    ...
    fun scoreEntrySheet_acceptsManualInput() {
        ...
        // Enter points
        composeTestRule.onNodeWithText("Points").performTextInput("15")
    ...
    fun scoreEntrySheet_showsUndoButton_afterAddingScore() = runBlocking {
        // Add a score first
        repository.addPoints(testPlayer.id, 10, 1)
        ...
        composeTestRule.waitUntil(timeoutMillis = 2000) {
            composeTestRule.onAllNodesWithText("Undo Last (+10)")
    ...
    fun scoreEntrySheet_filtersNonNumericInput() {
        ...
        composeTestRule.onNodeWithText("Points").performTextInput("abc123def")

        // Only numeric characters should remain
        // This is harder to test directly, but the button should be enabled if "123" was accepted
        composeTestRule.onAllNodesWithText("Add")[0].assertIsEnabled()
    }
}